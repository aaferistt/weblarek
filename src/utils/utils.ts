...
export function setElementData<T extends Record<string, unknown> | object>(
  ...
  for (const key in data) {
    el.dataset[key] = String((data as any)[key]);
  }
}
...
export function getElementData<T extends Record<string, unknown>>(
  ...
  for (const key in el.dataset) {
    // @ts-expect-error — ключи приходят строками
    data[key as keyof T] = scheme[key](el.dataset[key as keyof DOMStringMap]);
  }
  return data as T;
}
  ...
  if (props) {
    for (const key in props) {
      ...
      if (isPlainObject(value) && key === 'dataset') {
        ...
        // @ts-expect-error — индексируем по ключу элемента
...
/** Форматирование цены: 150000 -> '150 000' */
